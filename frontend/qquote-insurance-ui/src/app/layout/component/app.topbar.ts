    ...
    <div class="layout-topbar">
      ...
      <div class="layout-topbar-actions">
        <div class="layout-config-menu">
          <!-- Toggle dark mode (acción directa, sin menú) -->
          <button
            type="button"
          ...
          >
            <i [ngClass]="layoutService.isDarkTheme() ? 'pi pi-sun' : 'pi pi-moon'"></i>
          </button>

          <!-- User menu con p-menubar -->
          <p-menubar [model]="userProfileMenu" styleClass="topbar-user-menubar">
            <ng-template #item let-item let-root="root" let-hasSubmenu="hasSubmenu">
              @if (root) {
                <!-- Item raíz: el botón redondo del topbar -->
                <a
                  class="layout-topbar-action"
                  [attr.aria-label]="item.ariaLabel || 'Usuario'"
                  role="button"
                >
                  <i [ngClass]="item.icon"></i>
                  @if (item.items) {
                    <i class="pi pi-angle-down ml-1" style="font-size: 0.6rem"></i>
                  }
                </a>
              } @else {
                <!-- Items del submenú: label + ícono normal -->
  ...
  styles: [
    `
      :host ::ng-deep .topbar-user-menubar {
        ...
        /* el contenedor de la lista raíz */
        .p-menubar-root-list {
          gap: 0.35rem;
        }

        /* item raíz: el botón visible (icono usuario) */
        .p-menubar-item {
          position: relative;
        }

        /* nada de estilos default sobre nuestros botones */
        .p-menubar-item-link {
          padding: 0;
          background: transparent;
        }
        ...
        /* el submenú flotante sí mantiene su estilo de PrimeNG */
        .p-menubar-submenu {
          top: calc(100% + 0.5rem);
          right: 0;
          left: auto;
...
export class AppTopbar {
  ...
  /**
   * Mapeo opcional para mostrar labels "bonitos" cuando el segmento
   * de URL no coincide con el texto que queremos mostrar.
   */
  ...
  constructor() {
    // Construcción inicial (al primer render)
    this.buildBreadcrumb(this.router.url);

    // Reconstrucción en cada navegación
    this.router.events
      .pipe(
  ...
  private buildBreadcrumb(url: string): void {
    ...
    let path = '';
    const items: MenuItem[] = segments.map((segment, idx) => {
      path += '/' + segment;
      const isLast = idx === segments.length - 1;

      return {
        label: this.labelMap[segment] ?? this.humanize(segment),
        ...(isLast ? {} : { routerLink: path }),
      };
    });
